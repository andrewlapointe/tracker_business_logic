...
setup() ->
    ...
    case registration_server:start_link() of
        {ok, _Pid} -> ok;
...
register_package_success_test() ->
    ...
    {ok, "Package registered"} = registration_server:register_package(PackageData),
...
register_package_failure_test() ->
    ...
    %% Call the registration function
    {error, <<"Some error">>} = registration_server:register_package(PackageData),
